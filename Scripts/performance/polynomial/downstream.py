#!/usr/bin/env python3
"""Pair unchanged downstream Swift against native full-series/polynomial C and retain full payloads."""
import argparse
import gzip
import json
import math
...
def build():
    frozen=json.loads((ROOT/'Scripts/performance/native-math/results/downstream-inputs.json').read_text())
    ...
    # Only the native evaluator differs; downstream event/search experiment modes stay off.
    ...
    for kind in ['scoring','scan','events']:
        ...
        probe=ROOT/'Scripts/performance/native-math'/(kind+'-probe.swift')
...
def benchmark(scenarios=None):
    report=read('downstream-timings.json') if scenarios else {}
    historical={'scoring':.130250958,'nyc-day':.038368666,'nyc-week':.166827083,'reykjavik-day':.036499125}
    for scenario in (scenarios or historical):
        ...
        for trial in range(5):
            for name in NAMES[::(-1 if trial%2 else 1)]:
                ...
                p=subprocess.run(['/usr/bin/time','-l',str(OUT/(('scoring-' if scenario=='scoring' else 'scan-')+name)),scenario],
                ...
                assert rss,p.stderr
                data.update(processSeconds=elapsed,exitCode=p.returncode,peakRSSBytes=int(rss[1]))
...
if __name__=='__main__':
    ...
    p.add_argument('mode',choices=['build','events','benchmark'])
    p.add_argument('--scenarios',nargs='+',choices=['scoring','nyc-day','nyc-week','reykjavik-day'])
    a=p.parse_args()
    if a.mode=='benchmark': benchmark(a.scenarios)
    else: globals()[a.mode]()
