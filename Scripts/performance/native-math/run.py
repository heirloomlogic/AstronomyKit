#!/usr/bin/env python3
"""Build, validate and benchmark native math without changing archived experiments."""
import argparse
import ctypes as C
import gzip
import hashlib
...
def build():
    ...
    for name in NAMES:
        ...
        math_sources = sorted((croot/'detmath').glob('*.c')) if name=='baseline' else [croot/'ak_math.c']
        ...
        cmd = ['clang','-O2','-dynamiclib' if sys.platform=='darwin' else '-shared','-fPIC','-pthread',
               ...
               ROOT/'Scripts/accuracy/sample.c',HERE/'probe.c','-lm','-o',lib]
        invoke(cmd)
        records['libraries'][name] = {'path':str(lib),'command':list(map(str,cmd)), 'binarySHA256':sha(lib), 'sourceSHA256':sha(source)}
        if sys.platform=='darwin':
            module = croot/'module.modulemap'
            cmd = ['swiftc','-O','-whole-module-optimization','-I',croot,'-Xcc',f'-fmodule-map-file={module}',
                   *sorted((ROOT/'Sources/AstronomyKit').glob('*.swift')), HERE/'swift-probe.swift',lib,'-o',OUT/('swift-'+name)]
            invoke(cmd)
