/*
 * Vendored from musl libc 1.2.5 (src/math/hypot.c) for AstronomyKit.
...
double ak_hypot(double x, double y)
{
	...
	x = ux.f;
	y = uy.f;
	/* note: ak_hypot(inf,nan) == inf */
	if (ey == 0x7ff)
		return y;
	if (ex == 0x7ff || uy.i == 0)
		return x;
	/* note: ak_hypot(x,y) ~= x + y*y/x/2 with inexact for small y/x */
