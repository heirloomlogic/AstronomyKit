/*
 * Vendored from musl libc 1.2.5 (src/math/sin.c) for AstronomyKit.
 ...
 *
 * kernel function:
 *      ak___sin            ... sine function on [-pi/4,pi/4]
 *      ak___cos            ... cose function on [-pi/4,pi/4]
...
double ak_sin(double x)
{
	...
	if (ix <= 0x3fe921fb) {
		...
		return ak___sin(x, 0.0, 0);
	}

	/* ak_sin(Inf or NaN) is NaN */
	if (ix >= 0x7ff00000)
		return x - x;
	...
	n = ak___rem_pio2(x, y);
	switch (n&3) {
	case 0: return  ak___sin(y[0], y[1], 1);
	case 1: return  ak___cos(y[0], y[1]);
	case 2: return -ak___sin(y[0], y[1], 1);
	default:
		return -ak___cos(y[0], y[1]);
	}
}
