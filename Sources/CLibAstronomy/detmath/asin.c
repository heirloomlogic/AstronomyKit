/*
 * Vendored from musl libc 1.2.5 (src/math/asin.c) for AstronomyKit.
...
/* ak_asin(x)
 * Method :
 *      Since  ak_asin(x) = x + x^3/6 + x^5*3/40 + x^7*15/336 + ...
 *      we approximate ak_asin(x) on [0,0.5] by
 *              ak_asin(x) = x + x*x^2*R(x^2)
 *      where
 *              R(x^2) is a rational approximation of (ak_asin(x)-x)/x^3
 *      and its remez error is bounded by
 *              |(ak_asin(x)-x)/x^3 - R(x^2)| < 2^(-58.75)
 *
 *      For x in [0.5,1]
 *              ak_asin(x) = pi/2-2*ak_asin(sqrt((1-x)/2))
 *      Let y = (1-x), z = y/2, s := sqrt(z), and pio2_hi+pio2_lo=pi/2;
 *      then for x>0.98
 *              ak_asin(x) = pi/2 - 2*(s+s*z*R(z))
