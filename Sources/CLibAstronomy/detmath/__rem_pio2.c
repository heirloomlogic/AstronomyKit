/*
 * Vendored from musl libc 1.2.5 (src/math/__rem_pio2.c) for AstronomyKit.
 ...
 * use ak___rem_pio2_large() for large x
 */
...
int ak___rem_pio2(double x, double *y)
{
...
medium:
	...
	while (tx[i] == 0.0)
		i--;
	n = ak___rem_pio2_large(tx,ty,(int)(ix>>20)-(0x3ff+23),i+1,1);
	if (sign) {
		y[0] = -ty[0];
		y[1] = -ty[1];
		return -n;
