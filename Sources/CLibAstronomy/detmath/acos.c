/*
 * Vendored from musl libc 1.2.5 (src/math/acos.c) for AstronomyKit.
...
/* ak_acos(x)
 * Method :
 *      ak_acos(x)  = pi/2 - ak_asin(x)
 *      ak_acos(-x) = pi/2 + ak_asin(x)
 * For |x|<=0.5
 *      ak_acos(x) = pi/2 - (x + x*x^2*R(x^2))     (see ak_asin.c)
 * For x>0.5
 *      ak_acos(x) = pi/2 - (pi/2 - 2asin(sqrt((1-x)/2)))
 *              = 2asin(sqrt((1-x)/2))
